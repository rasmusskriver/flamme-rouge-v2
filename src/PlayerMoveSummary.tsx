...
export function PlayerMoveSummary({
  moves,
  riders,
  currentRound,
  isPlayer1,
  onContinue,
}: PlayerMoveSummaryProps) {
  return (
    <div className="mt-4 text-center">
      <h3 className="text-3xl font-bold mb-6 text-slate-200">
        Dine valg for runde {currentRound}
      </h3>
      <ul className="space-y-4 max-w-md mx-auto">
        {moves.map((m) => {
          ...
          return (
            <li
              key={m.rider_id}
              className="flex items-center justify-between gap-4 bg-slate-700 p-4 rounded-lg"
            >
              <span className="font-bold text-lg text-white">
                {rider?.color} {rider?.rider_type}
              </span>
              <span className="font-mono text-yellow-400 text-xl">
                {m.selected_card}
              </span>
            </li>
          );
